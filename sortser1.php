...
echo "<option value='" . $row['service_no'] ."'>" . $row['service'] ." </option>";
}
echo "</select>";
    ?>
...
$sql_qry = "SELECT sum(amount) as total FROM ser_pay_history where service_no=$s ";

$duration = $conn->query($sql_qry);
$record = $duration->fetch_array();
$total = $record['total'];
$sql_1 = "SELECT count(*) as total FROM ser_pay_history where service_no=$s ";

$duration1 = $conn->query($sql_1);
$record1 = $duration1->fetch_array();
$total1 = $record1['total'];

$sql = "SELECT * FROM ser_pay_history where service_no=$s ";
$result = $conn->query($sql);
?>
